...
impl Fetcher {
    pub(crate) fn new(url: &str, auth: Auth) -> Result<Self> {
        if auth == Auth::None {
            return Err(anyhow::anyhow!("No rpc authentication provided"));
        }
        ...
        let url = Uri::try_from(&url).map_err(|e| anyhow::anyhow!("Invalid rpc url {url}: {e}"))?;
    ...
    pub(crate) async fn get_transactions(&self, txids: Vec<Txid>) -> Result<Vec<Transaction>> {
        ...
        let txs = results
            ...
            .map(|res| {
                res.result
                    .ok_or_else(|| anyhow::anyhow!("Missing result for batched JSON-RPC response"))
                    .and_then(|str| {
                        hex::decode(str).map_err(|e| {
                            anyhow::anyhow!(
                                "Result for batched JSON-RPC response not valid hex: {e}"
                            )
                        })
                    })
                    .and_then(|hex| {
                        bitcoincore_rpc::bitcoin::consensus::deserialize(&hex).map_err(|e| {
                            anyhow::anyhow!(
                                "Result for batched JSON-RPC response not valid bitcoin tx: {e}"
                            )
                        })
                    })
            })
