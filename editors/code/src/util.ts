...
export function isClarityDocument(document: vscode.TextDocument): document is RustDocument {
	return document.languageId === 'clar'
		// SCM diff views have the same URI as the on-disk document but not the same content
		&& document.uri.scheme !== 'git'
		&& document.uri.scheme !== 'svn';
}

export function isClarityEditor(editor: vscode.TextEditor): editor is ClarityEditor {
