//! Installs clarity-lsp language server and/or editor plugin.
...
fn install_client(ClientOpt::VsCode: ClientOpt) -> Result<()> {
    ...
    let installed_extensions = if cfg!(unix) {
        ...
        run!("{} --install-extension clarity-lsp.vsix --force", code)?;
    ...
    if !installed_extensions.contains("clarity-lsp") {
        bail!(
...
fn install_server(opts: ServerOpt) -> Result<()> {
    ...
    if old_rust {
        eprintln!(
            "\nWARNING: at least rust 1.{}.0 is required to compile clarity-lsp\n",
